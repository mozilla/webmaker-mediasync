...
function getAuthToken( callback, region ) {
  ...
  request({
    ...
    if ( !CACHED_FILES_URL ) {
      return callback( "[webmaker-mediasync]: No Files url found" );
    }
  ...
  service: function( options, callback ) {
    ...
    function getContainerFiles( err ) {
      ...
      request({
        ...
        // Because of assumptions made about both an MP4 and WEBM file being uploaded with
        // the same file names, we filter out the results to be only WEBM videos
        body = _.filter( body, function( file ) {
          return file.content_type === "video/webm";
        });
    ...
    return function( req, res ) {
      ...
      function getContainerNames( err ) {
        ...
        request({
          ...
          uri: CACHED_FILES_URL + "?enabled_only=false"
        }, function( err, response, body ) {
